...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //Lab: hide the countdown label initially
    ...
    var captureTimer: Timer? // Timer for the 3 seconds countdown
    let captureDuration: TimeInterval = 3.0 // Duration for capturing images
    ...
    @IBAction func startCapturingButtonTapped(_ sender: UIButton) {
        startCountdown(from: 3) // Start the countdown for 3 seconds
    ...
    func startCountdown(from seconds: Int) {
        ...
        Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { timer in
            ...
            if countdown == 0 {
                ...
                //keep the Done label visible for 2 sec before countdown disappears
    ...
    //Lab: start the capturing process for computing mouth widths
    func startCapturingMouthWidths() {
        
        mouthWidths.removeAll()      // Reset the array for fresh capture
        captureTimer?.invalidate()   // Invalidate the previous timer if it exists
        
        // Start the timer for 3 seconds to take 50 captures
        captureTimer = Timer.scheduledTimer(withTimeInterval: captureDuration / 50.0, repeats: true) { [weak self] timer in
            guard let self = self else { return }
            // actual capturing will happen in the landmarksCompletionHandler
        }
        
        // Stop capturing after the duration
        DispatchQueue.main.asyncAfter(deadline: .now() + captureDuration) { [weak self] in
            self?.captureTimer?.invalidate()      // Stop the capture timer
            self?.calculateAverageMouthWidth()    // Calculate the average mouth width after capturing
        }
    }
    ...
    func captureMouthWidth(mouthWidth: CGFloat) {
        mouthWidths.append(mouthWidth) // Store the most recent calculated mouth width
    ...
    // Make sure to stop the timer when the view is deinitialized
    deinit {
            ...
            for faceObservation in results {
                if let landmarks = faceObservation.landmarks{
                    ...
                    if let mouth = landmarks.outerLips {
                        ...
                        // Call capture with the calculated width
                        self.captureMouthWidth(mouthWidth: mouthWidth)
                        
                        // Calculate the vertical position of corners
                        let leftCornerY = leftCorner.y
                        let rightCornerY = rightCorner.y
                        let mouthCenterY = (leftCornerY + rightCornerY) / 2
                        
                        // Define smile detection criteria
                        let smileThreshold: CGFloat = 0.1  // Threshold for smiling based on height change
                        
                        // Determine if the face is smiling
                        if mouthWidth > self.neutralMouthWidth + smileThreshold && leftCornerY < mouthCenterY && rightCornerY < mouthCenterY {
                            print("The face is smiling!")
                        } else {
                            print("The face is not smiling.")
                        }
                    }
                }
